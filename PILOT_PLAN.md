...
- [ ] Before baseline, the pilot registry freezes opaque rep IDs, cohort, matching strata, and active dates; when each baseline-anchor or index evaluation is created, orchestration records its CRM owner snapshot, and meeting events later reconcile to the index snapshot.
...
The baseline anchor evaluation is the first eligible read-only shadow evaluation for a contact during the ten-business-day pre-period. It never emits `score.shown` and never writes to CRM. The index evaluation is the first eligible evaluation for that contact during exposure. Contact-level outcome metrics use the index evaluation so repeated scoring does not multiply the denominator. Recommendation-disposition metrics remain evaluation-level because each displayed recommendation is a separate decision.

Before baseline capture, freeze a customer-approved pilot registry keyed only by opaque IDs. Its cohort portion maps each participating rep to cohort, matching strata, and active dates. When a baseline-anchor or index evaluation is created, record its CRM owner snapshot in an append-only ownership ledger keyed to that evaluation; do not try to prepopulate future evaluation IDs. Event `actorId` identifies the event actor and must not be assumed to be the assigned rep. For `meeting.attribution`, `occurredAt` is the source meeting-booking timestamp, not ingestion time; the CRM extract supplies meeting creation/association details and the frozen registry supplies active rep-weeks.
...
| Hot false-positive rate | Count distinct eligible treatment contacts whose Hot index evaluation is later linked to `outcome.attribution` of `bad_fit` or `disqualified`; divide by distinct eligible treatment contacts with a Hot `score.shown` for their index evaluation. Use one index evaluation and the first qualifying outcome per contact, retaining attribution basis. | Hot treatment index contacts with a full 60-day outcome window. Incomplete attribution is reported separately. | Customer's pre-launch native priority/Hot definition, documented before exposure; 25% relative reduction and below baseline. Shadow-control index `evaluation.run` is a separate diagnostic because control has no truthful `score.shown`. | Product analytics; RevOps approves native-status mapping. |
...
| Email-open primary-driver rate | Count distinct Hot index evaluations having an email-open `source.contribution` with `weakSignal=true` and `hotMaking=true`; divide by distinct Hot index evaluation runs. `hotMaking` requires deterministic scoring under the same score/config version to produce a non-Hot band when only the referenced open contribution is removed. A primary label or the presence of opens alone is insufficient. Other weak-signal types are reported separately and do not enter this PRD metric. | Hot index evaluations during exposure, segmented by score/config version. The preflight must prove every counted contribution resolves to evaluation-owned normalized open evidence and its same-version counterfactual. | Shadow baseline under the frozen configuration; below admin-approved threshold, initially 10%. | Product analytics; RevOps approves threshold and investigates source mix. |
...
| Baseline | Instrument |
| --- | --- |
| Research time | Create one read-only baseline anchor per eligible contact and run the same `lead.viewed` to `action.first_meaningful` capture for participating reps during at least ten business days before exposure. Do not emit `score.shown`, write to CRM, or substitute an industry benchmark. |
...
| Conversion | For each contact in a prespecified historical intake window ending at least 60 days before exposure, use the first CRM assignment to a participating rep as the historical anchor and snapshot owner, lifecycle stage, and lead source from CRM property history at that time. If assignment history is unavailable, use contact creation only when the same owner at creation is provable; otherwise exclude the contact and report missingness. Apply the same eligibility and meeting-deduplication rules, observe outcomes for 60 days after the anchor, and report by lead source and incumbent priority band. |
...
- **Proceed:** no unresolved safety stop; primary-metric coverage is adequate; research time falls at least 40% from the treatment baseline and its change is more favorable than control; acceptance reaches at least 60%; completeness improves at least 20% from the treatment baseline and its change is more favorable than control; Hot false positives fall at least 25% relative to baseline; email-open-driven Hot records stay below the approved threshold; bad writebacks remain below 1%; trust reaches at least 70% among valid respondents with responses from at least 70% of enrolled treatment reps; and the native-alternative honest gate passes. Meeting lift supports the decision after maturation but is not required to clear a low-powered pilot alone.
...
- **Stop:** any critical safety issue remains unresolved; either research time or acceptance misses without a credible correctable reason or measurement fault; RevOps cannot reproduce/audit scoring; CRM harm is unacceptable; or the native/single-vendor honest gate fails.
