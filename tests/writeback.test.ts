...
import { executeWriteback, hubSpotWritebackPolicy, planWriteback, rollbackWriteback } from "../src/lib/writeback.ts";
...
test("planning maps canonical evidence to CRM fields independently per field", () => {
  ...
  assert.equal(plan.fields.find(({ canonicalField }) => canonicalField === "employees")?.outcome, "Written");
...
test("execution is dry-run-first and retries do not repeat writes", async () => {
  ...
  try {
    const plan = planWriteback(lead, hubSpotWritebackPolicy);
    const dryRun = await executeWriteback(plan, { store, tenantId: "tenant-1", actorType: "system", write: async () => { writes += 1; } });
    assert.ok(dryRun.every(({ outcome }) => outcome === "Skipped"));
    assert.equal(writes, 0);
    await assert.rejects(executeWriteback(plan, { store, tenantId: "tenant-1", actorType: "system", mode: "live", authorizedLiveWrite: true, write: async () => { writes += 1; } }), /not explicitly enabled/i);

    const livePlan = planWriteback(lead, { ...hubSpotWritebackPolicy, liveWritesEnabled: true });
    const options = { store, tenantId: "tenant-1", actorType: "system", mode: "live" as const, authorizedLiveWrite: true, write: async () => { writes += 1; } };
    const first = await executeWriteback(livePlan, options);
...
test("field rollback writes the previous value and preserves an immutable audit chain", async () => {
  ...
  try {
    ...
    const written = await executeWriteback(planWriteback(lead, policy), {
      store, tenantId: "tenant-1", actorType: "system", mode: "live", authorizedLiveWrite: true,
      write: async (call) => { calls.push(call); }
    });
    const audit = written.find(({ field_name }) => field_name === "numberofemployees")!;
    await rollbackWriteback([audit.audit_id!], {
      store, tenantId: "tenant-1", actorType: "admin", authorizedLiveWrite: true,
      write: async (call) => { calls.push(call); }
    });
    assert.equal(calls.length, written.length + 1);
    assert.deepEqual(calls.at(-1), { object: "company", objectId: lead.account_id, properties: { numberofemployees: null } });
