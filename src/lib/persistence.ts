  ...
  appendAuditRecord(input: AuditRecord) {
    ...
    this.database.prepare(`
      INSERT INTO writeback_audit_records (
        ...
        outcome, reason, score_version, policy_version, actor_type, recorded_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      ...
      input.sourceUpdatedAt, input.confidence, input.outcome, input.reason, input.scoreVersion, input.policyVersion ?? input.scoreVersion, input.actorType,
